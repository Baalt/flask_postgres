CREATE OR REPLACE FUNCTION chk_new_user_for_duplication_and_ip_attack_then_add(username varchar,
								      pswd varchar,
...
CREATE OR REPLACE FUNCTION insert_into_referral_parent_inheritor_id(parent_referral_key varchar, client_email varchar)
RETURNS void AS $$
DECLARE
	parent_id integer;
...
CREATE OR REPLACE FUNCTION chk_url_change_reg_status(url_reg_repair_key varchar, new_reg_repair_key varchar)
RETURNS varchar AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION chk_email_return_username_pswd_referral_parent_inheritor(client_email varchar) RETURNS user_status AS $$
DECLARE
